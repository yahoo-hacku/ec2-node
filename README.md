...
- When developing locally, the single command `bin/hacku-httpd.mv` runs your web server and takes care of serving static files
- When deploying changes (after a git push), `hacku-update restart` deploys changes and restarts services on your server
- Rolling back the server to an earlier version is a simple as `hacku-update restart v0.1.2`
